...
export interface AllocateInput {
  ceilingKw: number;
  /** EEG §9 max export kW (Infinity if unconstrained). */
  maxFeedInKw?: number;
  state: PlantState;
...
/**
 * Propose setpoints under §14a ceiling (+ pass-through §9 feed-in cap).
 * Priority default: heat pump > wallbox (if deadline soon) > MultiPlus grid charge.
 */
export function allocateUnderCeiling(input: AllocateInput): PowerSetpoints {
  const { ceilingKw, state, wish, limits } = input;
  const now = input.now ?? new Date();
  const maxFeedInKw =
    input.maxFeedInKw === undefined ? Number.POSITIVE_INFINITY : input.maxFeedInKw;
  ...
  const hpMax =
    wish.heatPump?.preferOn === false ? 0 : limits.maxHeatPumpKw;
  ...
  const heatPump = heatPumpCommandFromPower(
    powers.heatPumpKw,
    limits,
    wish.heatPump?.preferredLevel,
  );

  return {
    heatPumpKw: powers.heatPumpKw,
    heatPump,
    wallboxKw: powers.wallboxKw,
    batteryGridChargeKw: powers.batteryGridChargeKw,
    maxFeedInKw,
  };
}
