...
- API service: `npm start -w @victron-ems14a/api` → HTTP on `http://127.0.0.1:8787`. Every request needs an `X-Role: enduser|installer|system` header; RBAC is enforced (e.g. `enduser` gets `403` on `grid:write`). Routes: `GET /health`, `GET /status`, `POST /grid/signal`, `POST /feedin/signal`, `PUT /comfort`, `POST /control/tick`.
